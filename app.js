...
function AuthModal({ onClose, onAuthenticate, theme }) {
  ...
  const handleEmailNext = (e) => {
    ...
    // Any email ending with @sst.scaler.com maps to Student Portal, all others map to TA Portal
...
function CampfireCredApp() {
  ...
  const [user, setUser] = useState(() => JSON.parse(localStorage.getItem("campfire_active_user")) || null);
  const [activeTab, setActiveTab] = useState(() => {
    const savedUser = JSON.parse(localStorage.getItem("campfire_active_user"));
    return savedUser?.isTA ? "ta_dashboard" : "feed";
  });
  ...
  const handleAuthenticate = (loggedInUser) => {
    ...
    // Redirect based on role: TAs go to TA Portal, Students go to Feed
    ...
    <div className={`min-h-screen ${theme.appBg}`}>
      ...
      {!user ? (
        <div className="text-center pt-24">
          <h1 className="text-4xl font-bold mb-4">Welcome to CampfireCred</h1>
          <button onClick={() => setShowAuth(true)} className="bg-amber-500 text-slate-950 font-bold px-6 py-3 rounded-xl">Sign In to Continue</button>
        </div>
      ) : (
